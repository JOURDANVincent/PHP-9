<?php
    ...
    if (!empty($_GET['home'])) { 
        $_SESSION = $_COOKIE = $month = $year = null;
        session_destroy();
    } 

    // récupère état affichage
    if (!empty($_GET['layout']) OR (!empty($_SESSION['layout']))) {
        $layout = $_SESSION['layout'] = 'show';
    } else {
        $layout = 'hide';
    }

    // récpuère l'année selectionnée
    if (!empty($_GET['year'])) {
    ...
    if (empty($_COOKIE['calendar'])) {
        ...
        $serialize_calendar = serialize($calendar);
        setcookie('calendar', $serialize_calendar);

    } else { // sinon on recupère le calendar
        $unserialize_calendar = $_COOKIE['calendar'];
        $calendar = unserialize($unserialize_calendar);
    ...
    if(!empty($_GET['newEventDescription']) OR !empty($_GET['newEventDay']) OR !empty($_GET['newEventType'])) { 
        ...
        $serialize_calendar = serialize($calendar);
        setcookie('calendar', $serialize_calendar);
        $_COOKIE['calendar'] = $serialize_calendar;

    }


    // ------------------------------------ calcul des différentes données calendrier -----------------------//
    ...
    if ($layout == 'show') {
        ...
        // --------------------- recherche événement dans le calendrier à afficher -----------------------------//
        ...
        foreach($calendar as $type => $value) {
            ...
            foreach($calendar[$type]->array as $key => $value) {  // recherche dans tableau birthday
                ...
                if ($month == $searchInCalendar[1] AND ($year == $searchInCalendar[2] OR $type == 'birthday')) {
                    ...
                    $searchInCalendar[0] = intval($searchInCalendar[0]);
        ...
        // construction du tableau avant affichage du calendrier (structure)
        $array = [];
        ...
        <div class="row align-items-center justify-content-center">
            ...
            <div class="col-6 mt-5">
                ...
                <form action="index.php" method="get">
                    ...
                    <input type="hidden" name="layout" value="show">
    ...
    <div class="container text-center mt-3">
        ...
        <table class="table table-dark">
            ...
            <div class="d-flex justify-content-end align-items-center">
                ...
                <form class="col-1 d-flex pr-0 justify-content-end" action="index.php" method="get">
                    <a href="index.php?month=<?=($month - 1);?>" title="mois précédent"><span class="prev-m mr-3 py-2 px-2">&#x25B2;</span></a>
                    <a href="index.php?month=<?=($month + 1);?>" title="mois suivant"><span class="next-m py-2 px-2">&#x25BC;</span></a>
                </form>  
            </div>
